...
- Project descriptions are written in **first person** ("I want", "I believe", "I built") per Anson's direct instruction — not third person ("Anson believes..."). All six entries in `src/content/projects.js` are first person; SignalFeed and Algo Trading Bot were converted on 2026-08-15 (pronoun/voice only, content unchanged, per Anson's explicit go-ahead).
- `src/content/projects.js` — SignalFeed, Algo Trading Bot, AI Newsletter, and Daily Gap Ups are real, live projects (Algo Trading Bot is backtested only, not traded live — see its `status`; AI Newsletter and Daily Gap Ups are both live scheduled agents that email Anson automated reports — a daily AI intelligence brief and a pre-market gap report respectively — per his direct instruction on 2026-08-15). Both have real screenshots of the actual emails they send — AI Newsletter uses two (`AI Newsletter NEW (1).png` / `(2).png`, swapped in 2026-08-16 replacing the old single `AI Newsleter.png`), Daily Gap Ups uses three (`Gap Ups New pictures  (1/2/3).png`, swapped in 2026-08-21 replacing the old single `Gap Ups.png`, now that the agent sends two separate alerts — the gap-up list itself and a follow-up with the catalyst detail). P&L Tracker for Rideshare Drivers is Developing; Compersion and Solace was promoted to Testing per Anson's direct instruction (2026-08-21). The P&L Tracker's and Compersion and Solace's descriptions are Claude-authored drafts awaiting Anson's rewrite — flagged inline in the file. Earnings Signal Agent was removed from the pipeline entirely per Anson's direct instruction (2026-08-21). Each entry's `stageIndex`/`pipelineSummary` feed `PipelineTable.jsx` (see Component registries above).
