...
- Project descriptions are written in **first person** ("I want", "I believe", "I built") going forward, per Anson's direct instruction — not third person ("Anson believes..."). Applied so far to P&L Tracker, Compersion and Solace, and AI News Dashboard; SignalFeed, Algo Trading Bot, and Earnings Signal Agent still read third-person and haven't been touched — ask before converting them too, don't do it silently.
- `src/content/projects.js` — SignalFeed and Algo Trading Bot are real, live-ish projects with real screenshots (Algo Trading Bot is backtested only, not traded live — see its `status`). Earnings Signal Agent, P&L Tracker for Rideshare Drivers, Compersion and Solace, and AI News Dashboard are all pre-build/idea stage. The P&L Tracker's and Compersion and Solace's descriptions are Claude-authored drafts awaiting Anson's rewrite — flagged inline in the file. Each entry's `stageIndex`/`pipelineSummary` feed `PipelineTable.jsx` (see Component registries above).
