...
function ImageCarousel({ project, imgStyle }) {
  const [idx, setIdx] = useState(0)
  const images = project.images ?? []
  const multi = images.length > 1

  return (
    <div style={{ position: 'relative', background: '#0d0d0d', ...imgStyle }}>
      <img
...
function ProjectCard({ project, index }) {
    ...
    return (
      <article style={divider}>
        <div style={{
          ...
          {/* Portrait image panel — fixed width so it stays narrow */}
          <ImageCarousel
            project={project}
            imgStyle={{ flex: '0 0 320px', height: 560 }}
          />
          {/* Text beside it */}
  ...
  return (
    <article style={divider}>
      <ImageCarousel
        project={project}
        imgStyle={{ width: '100%', maxHeight: '60vh' }}
      />
