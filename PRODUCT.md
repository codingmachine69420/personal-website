...
- Home and Projects & Experiences heroes play a slow, one-shot fade/slide-up reveal on mount (`framer-motion`, `duration: 1.2–1.5s`, ease `[0.16, 1, 0.3, 1]` — the same easing curve used in `index.css` for `.film-frame`). Respected by `MotionConfig reducedMotion="user"` automatically. Reuse this pattern (`revealText`/`revealPhoto`/`revealHero` objects, spread onto a `motion.div`) for future hero-level sections rather than inventing a new one.
...
- **Interests** (`/interests`): Hub page → Cinema (`/interests/cinema`) watchlist with TMDB posters, star ratings, CLASSICS ONLY badge. More interest categories to follow. The paper-background body section has a decorative tattoo-flash-style icon scatter behind the content (`src/components/TattooScatter.jsx` — flower, basketball, running shoe line-art in accent ochre / ink, low opacity, `pointer-events: none`). Running shoe stands in for Marathon Training — flagged as an interpretation, swap if Anson meant a different hobby (e.g. Viola/Orchestra).
