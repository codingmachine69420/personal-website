...
- `src/components/CollageBanner.jsx`: reusable scattered-photo-collage banner (used on Mindset and Projects & Experiences in place of the old striped "Photo banner — coming" placeholder). Each photo gets its own size/rotation/vertical-offset/shadow ("elevation") plus a hand-picked `objectPosition` so cropping never hides the actual subject — never default to `pos: 'center center'` when adding a photo here without looking at it first. One photo per banner gets `accent: true` for an ochre border instead of paper-white.
