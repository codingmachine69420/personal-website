...
- Neither Home nor Projects & Experiences uses framer-motion for an entrance reveal anymore — Home's was removed after the photo frame's entrance-scale read as an unwanted pop; Projects & Experiences' accent-bar-plus-description block (which had the same reveal) was later removed outright per feedback, taking the `motion`/`revealHero` code with it. Don't re-add hero-level motion to either page without checking first.
