...
- **Mindset** (`/mindset`): 4-column card grid. Real essay content for High Agency, Value Driven, Delusional Optimism, Curiosity. Four placeholder cards in row 2. Do not invent new essay text. Clicking a real card no longer expands an inline panel below the row — it shared-layout-morphs (Watermelon `expandable-event-card` pattern) into a full-screen reader, which has a "Keep reading" strip (Watermelon `minimal-carousel` pattern) to switch essays without closing.
